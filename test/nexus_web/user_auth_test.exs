defmodule NexusWeb.UserAuthTest do
  ...
  @remember_me_cookie "_nexus_web_user_remember_me"
  ...
  describe "log_in_user/3" do
    ...
    test "writes the remember me cookie", %{conn: conn, user: user} do
      conn = conn |> fetch_cookies() |> UserAuth.log_in_user(user)
      assert get_session(conn, :user_token) == conn.cookies[@remember_me_cookie]

      assert %{value: signed_token, max_age: max_age} = conn.resp_cookies[@remember_me_cookie]
      assert signed_token != get_session(conn, :user_token)
      assert max_age == 10_540_800
    end
  end

  describe "logout_user/1" do
    test "erases session and cookies", %{conn: conn, user: user} do
      user_token = Accounts.create_user_session_token(user)
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      NexusWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_authenticated_user/2" do
    test "redirects if user is not authenticated", %{conn: conn} do
      ...
      assert conn.halted
      assert redirected_to(conn) == Routes.live_path(conn, NexusWeb.RequestLoginLive)
